...
func (r *MsgpReader) Read() (Type, int, []byte, error) {
	...
	if msgpType == Nil {
		...
		r.idx++
		return msgpType, 0, r.Buffer[r.idx-1 : r.idx], nil
	} else if msgpType == Int16 || msgpType == Uint16 {
		r.idx += 2
		return msgpType, 0, r.Buffer[r.idx-2 : r.idx], nil
	} else if msgpType == Int32 || msgpType == Uint32 || msgpType == Float32 {
		r.idx += 4
		return msgpType, 0, r.Buffer[r.idx-4 : r.idx], nil
	} else if msgpType == Int64 || msgpType == Uint64 || msgpType == Float64 {
		r.idx += 8
		return msgpType, 0, r.Buffer[r.idx-8 : r.idx], nil
	} else if msgpType == Bin8 || msgpType == Ext8 || msgpType == Str8 {
		length := int(r.Buffer[r.idx])
		r.idx++
		return msgpType, 0, r.Buffer[r.idx : r.idx+length], nil
	} else if msgpType == Bin16 || msgpType == Ext16 || msgpType == Str16 {
		length := int(r.Buffer[r.idx])<<8 | int(r.Buffer[r.idx+1])
		r.idx += 2
		return msgpType, 0, r.Buffer[r.idx : r.idx+length], nil
	} else if msgpType == Bin32 || msgpType == Ext32 || msgpType == Str32 {
		length := int(r.Buffer[r.idx])<<24 | int(r.Buffer[r.idx+1])<<16 | int(r.Buffer[r.idx+2])<<8 | int(r.Buffer[r.idx+3])
		r.idx += 4
		return msgpType, 0, r.Buffer[r.idx : r.idx+length], nil
	} else if msgpType == FixExt1 {
		r.idx++
		return msgpType, 0, r.Buffer[r.idx : r.idx+1], nil
		...
		r.idx += 16
		return msgpType, 0, r.Buffer[r.idx-16 : r.idx], nil
	} else if msgpType >= FixArray && msgpType <= FixArrayMax {
		length := int(msgpType) - int(FixArray)
		return msgpType, length, r.Buffer[r.idx:], nil
